...
@celery.task
def send_email_task2(recipient, text: str):
    mailertogo_host = os.environ.get('MAILERTOGO_SMTP_HOST')
    mailertogo_port = os.environ.get('MAILERTOGO_SMTP_PORT', 587)
    mailertogo_user = os.environ.get('MAILERTOGO_SMTP_USER')
    mailertogo_password = os.environ.get('MAILERTOGO_SMTP_PASSWORD')
    ...
    prompt = f"Here is a series of completed therapy sessions with summaries and insights:" \
             f"\n\n{text}\n\n " \
    ...
    for i in range(2):
        try:
            ...
            <body>
                <h1>Insights Analysis from MindMateGPT :)</h1>
                <p>{message}</p>
            </body>
            </html>
            """.format(message=html_message)
            ...
            msg['From'] = sender_email
            msg['To'] = recipient
            msg['Subject'] = 'Insights Analysis from MindMateGPT :)'
